 ...
 **/
public class HommingAttackGoal extends Goal {
    ...
    public HommingAttackGoal(Mob owner, boolean explosive, float velocity, float inaccuracy, int delay) {
        mob = owner;
    ...
    public void tick() {
        ...
        if (target != null && mob.tickCount % arrowDelay == 0) {
            if (target.distanceToSqr(mob) < 4096 && mob.hasLineOfSight(target)) {
                ...
                level.playSound(null, target.getX(), target.getY(), target.getZ(), ContentRegistry.SHOOT.get(), SoundSource.BLOCKS, 1, 1);
                if (explosiveArrow) {
                    ExplosiveHommingArrow explosiveHommingArrow = new ExplosiveHommingArrow(EntityRegistry.EXPLOSIVE_HOMMING_ARROW.get(), mob, level);
                    explosiveHommingArrow.shoot(x, y, z, arrowVelocity, arrowInaccuracy);
                    level.addFreshEntity(explosiveHommingArrow);
                } else {
                    HommingArrow hommingArrow = new HommingArrow(EntityRegistry.HOMMING_ARROW.get(), mob, level);
