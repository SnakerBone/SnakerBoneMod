package snaker.snakerbone.client.layer;
...
public class CosmoLayer extends GeoLayerRenderer<Cosmo> {
    ...
    public void render(PoseStack ps, MultiBufferSource buffer, int light, Cosmo entity, float swing, float amount, float pt, float age, float yaw, float pitch) {
        ...
        getRenderer().render(getEntityModel().getModel(new SnakerBoneResourceLocation("geo/cosmo.geo.json")), entity, pt, SnakerBoneShaders.COSMO_VARIANT.get(entity.getVariant()), ps, buffer, null, light, LivingEntityRenderer.getOverlayCoords(entity, 0), 1, 1, 1, 1);
